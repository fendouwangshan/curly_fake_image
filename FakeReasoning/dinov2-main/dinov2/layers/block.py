...
class Block(nn.Module):
    ...
    #     if return_attention:
    #         attn_map = self.attn(self.norm1(x), return_attn=True)
    ...
    def forward(self, x: Tensor, return_attention=False):
        norm_x = self.norm1(x)
        if return_attention:
            attn_out, attn_map = self.attn(norm_x, return_attn=True)
            attn_out = self.ls1(attn_out)
        else:
            attn_out = self.ls1(self.attn(norm_x))
            attn_map = None

        def ffn_residual_func(x):
            return self.ls2(self.mlp(self.norm2(x)))

        # 其余代码保持不变，确保残差加和时使用 attn_out

        if self.training and self.sample_drop_ratio > 0.1:
            x = drop_add_residual_stochastic_depth(
                x,
                residual_func=lambda x: attn_out,
                sample_drop_ratio=self.sample_drop_ratio,
            )
            x = drop_add_residual_stochastic_depth(
                x,
