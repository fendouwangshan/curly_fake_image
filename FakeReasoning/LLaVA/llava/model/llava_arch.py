...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        self.crossattention = CrossAttention(atten_config, delay_load=False)
    ...
    def get_attention_model(self):
        attention_model = getattr(self, 'crossattention', None)
        return attention_model


    def initialize_vision_modules(self, model_args, fsdp=None):
        vision_tower = model_args.vision_tower  #vision encoder of clip
        mm_vision_select_layer = model_args.mm_vision_select_layer
        mm_vision_select_feature = model_args.mm_vision_select_feature
        pretrain_mm_mlp_adapter = model_args.pretrain_mm_mlp_adapter
        mm_patch_merge_type = model_args.mm_patch_merge_type
        pretrain_dino_mm_mlp_adapter = None
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_attention_model(self):
        return self.get_model().get_attention_model()
    ...
    def encode_images_withdino(self, images):
        image_features_dino,attn_map = self.get_model().get_vision_tower(load_model = "dino")(images)
        image_features_dino = self.get_model().dino_mm_projector(image_features_dino)
        return image_features_dino,attn_map    
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            clip_feature = self.encode_images_withclip(images)
            dino_feature, attn_map = self.encode_images_withdino(images)
            image_features = self.get_model().crossattention(clip_feature, dino_feature, attn_map)
